...
> = {
  ...
  autocompleteProps?: Omit<
    AutocompleteProps<TValue, Multiple, DisableClearable, FreeSolo, ChipComponent>,
...
>(
  ...
  const getOptionLabel = (option: TValue | AutocompleteFreeSoloValueMapping<FreeSolo>): string => {
                ...
                return (
                  matchId
                    ? newValues.map((currentValue) =>
                        hasOwnProperty(currentValue, "id") ? currentValue.id : currentValue,
                      )
                    : newValues
                ) as PathValue<TFieldValues, TName>;
              }
              return (
                matchId && hasOwnProperty(newValue, "id") ? newValue.id : newValue
              ) as PathValue<TFieldValues, TName>;
            },
    },
  });
  ...
  return (
    <Autocomplete
      size="small"
      {...autocompleteProps}
      sx={{ width: 300, ...(autocompleteProps?.sx || {}) }}
      value={value}
      loading={loading}
      multiple={multiple}
      options={options}
